...
namespace Shane.Church.StirlingMoney.WP
{
    public partial class App : Application
    {
        ...
        private void InitializeBackgroundAgent()
        {
            //try
            //{
            //	IAgentManagementService service = KernelService.Kernel.Locate<IAgentManagementService>();
            //	service.StartAgent();
            //}
        ...
        private void MergeCustomThemes()
        {
            var dictionaries = new ResourceDictionary();
            string source = String.Format("/Shane.Church.StirlingMoney.WP;component/ThemeResources.xaml");
            var themeStyles = new ResourceDictionary { Source = new Uri(source, UriKind.Relative) };
            dictionaries.MergedDictionaries.Add(themeStyles);


            ResourceDictionary AppResources = App.Current.Resources;
            foreach (KeyValuePair<object, object> entry in dictionaries.MergedDictionaries[0].Where(it=>it.Value is SolidColorBrush))
            {
