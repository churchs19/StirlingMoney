...
namespace Shane.Church.StirlingMoney.Core.WP
{
    public static class NinjectBootstrapper
    {
        public static void Bootstrap()
        {
            ...
            KernelService.Kernel.Rebind<ITransactionSearch>().To<TransactionRepository>();
            ...
            KernelService.Kernel.Rebind<SyncService>().To<WP8SyncService>();
            KernelService.Kernel.Rebind<ISterlingPlatformAdapter>().To<Wintellect.Sterling.WP8.PlatformAdapter>();
            KernelService.Kernel.Rebind<ISterlingDriver>().To<Wintellect.Sterling.WP8.IsolatedStorage.IsolatedStorageDriver>();
            ...
            KernelService.Kernel.Rebind<ILicensingService>().To<WP8LicensingService>().InSingletonScope();
            KernelService.Kernel.Rebind<IBackupService>().To<WP8BackupService>();
            KernelService.Kernel.Rebind<IUpgradeDBService>().To<PhoneUpgradeDBService>();
#else
            KernelService.Kernel.Rebind<ILicensingService>().To<WP8AgentLicensingService>().InSingletonScope();
#endif
        }
    }
}
